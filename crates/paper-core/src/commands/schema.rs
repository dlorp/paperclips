...
pub fn contract(target: SchemaTarget) -> Value {
    let records = json!({
        "cut": {"kind":"cut","id":"pc_<12 lowercase hex>","ts":"RFC3339 UTC milliseconds","agent":"string","text":"string <= 10000 bytes","tags":["string"],"severity":"minor|major|blocker","cwd":"repo-relative path or absolute if no repo","repo":"\".\" when inside a repo, null otherwise"},
    ...
    match target {
        ...
        SchemaTarget::All => json!({
            ...
            "commands": {
                "add": {"alias":["log"],"positional":"TEXT or -; optional when stdin is piped","flags":{"--agent":"NAME","--tag":"TAG; repeatable","--severity":"minor|major|blocker; default minor","--dry-run":"boolean","--force":"bypass secret scan"},"output":"{changed,record}","read_only":false,"appends":true,"destructive":false},
                ...
                "resolve": {"positional":"ID; optional pc_ plus at least 4 hex digits","flags":{"--note":"TEXT","--agent":"NAME","--dry-run":"boolean"},"output":"{changed,record-with-resolution}","read_only":false,"appends":true,"destructive":false},
